...
import { FileText, Calendar, Users, Trash2, CheckCircle, Clock, Eye, Download, Palette, Edit2, Check, X, ChevronLeft, ChevronRight, Copy, Search } from 'lucide-react';
...
interface OrdersListProps {
  ...
  onDuplicateOrder?: (orderId: string) => Promise<string | null>;
  onRefreshOrders?: () => void;
}

const OrdersList: React.FC<OrdersListProps> = ({ orders, onSelectOrder, onDeleteOrder, onDuplicateOrder, onRefreshOrders }) => {
  ...
  // Filtro de búsqueda: nombre del pedido, empresa, contacto, nombre de
  // archivo y fecha del evento. Insensible a mayúsculas/acentos.
  ...
  const filteredOrders = useMemo(() => {
    ...
    return orders.filter(order => {
      const haystack = [
        ...
        order.data?.client_details?.contact_person,
        order.fileName,
        order.data?.order_information?.event_date,
      ]
        .map((v) => normalize(v || ''))
        .join(' ');
  ...
  const handleDuplicate = async (e: React.MouseEvent, orderId: string) => {
    e.stopPropagation();
    if (!onDuplicateOrder || duplicatingId) return;
    setDuplicatingId(orderId);
    try {
      await onDuplicateOrder(orderId);
    } catch (err) {
      console.error('Error duplicando pedido:', err);
      alert('No se pudo duplicar el presupuesto. Inténtalo de nuevo.');
    } finally {
  ...
  const pageStart = filteredOrders.length === 0 ? 0 : (safePage - 1) * PAGE_SIZE + 1;
    ...
    <div className="w-full">
      ...
      <div className="mb-3">
        <div className="relative">
          ...
          <input
            ...
            placeholder="Buscar por cliente, contacto, archivo, fecha…"
            className="w-full pl-9 pr-9 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm"
          />
          {searchQuery && (
            <button
        ...
        <span>
          {searchQuery ? (
            <>
              ...
              resultado{filteredOrders.length !== 1 ? 's' : ''} de{' '}
      ...
      {filteredOrders.length === 0 ? (
        <div className="bg-white rounded-xl shadow-sm p-10 text-center">
          ...
          <p className="text-slate-600 font-medium">Sin resultados para “{searchQuery}”</p>
          <button
            onClick={() => setSearchQuery('')}
                ...
                <div className="flex gap-1.5 mt-auto">
                  ...
                  {onDuplicateOrder && (
                    <button
                      ...
                      className="px-2.5 py-2 bg-slate-100 text-slate-600 rounded-lg hover:bg-slate-200 hover:text-slate-800 transition-colors flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed"
